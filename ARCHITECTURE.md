...
- **Protected writes — `functions/admin/api/*`** (route `/admin/api/*`, behind Cloudflare Access):
  ...
  - `/admin/api/players` — onboard/add/remove player, refresh handicaps, `set-discord` (map a player to their numeric Discord user ID in `players:discord`, used for `<@id>` mentions in results posts, League Alert, and the approval digest), `check-one` (live single-player SGT `player-check` lookup — rawCap/comboCap/events/connector — used on `admin/registrations.html` to vet a registrant before approving; deliberately avoids `player-hcp-rounds`, which shares a 24h-per-key cache across the whole roster)
