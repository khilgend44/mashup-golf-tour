...
- **Not yet built:** more player/team stat ideas (bounce-back rate, consistency/variance, biggest single-hole blowup, CTP-hit tie-ins), and a real YouTube-stream-duration "fastest/slowest round" stat — floated but blocked on (a) a new `YOUTUBE_API_KEY` credential (none exists in this repo) and (b) confirming players actually record one video per round rather than both rounds back-to-back, or the "duration" would measure the stream, not the round.
...
| **YouTube Data API Key** | Cloudflare Pages → Settings → Environment Variables → `YOUTUBE_API_KEY` (set as **Secret**, not Text) | Unknown — Google API keys don't auto-expire, but rotate if leaked | Added 2026-09 for `/admin/api/youtube-duration` (video length lookups feeding "total time played"/"fastest round" stats on the Event Summary). Created under the existing `mashup-admin` Google Cloud project — no need for a separate project per API. Restricted to YouTube Data API v3 only. Free quota is 10,000 units/day; a duration lookup costs 1 unit, so normal usage never approaches the limit. |
