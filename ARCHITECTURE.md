...
- **`mashup-approval-digest`** — Cloudflare dashboard → Workers & Pages → mashup-approval-digest. Fires **daily at 17:12 UTC** (~1:12pm ET during daylight saving; deliberately off the top of the hour to dodge scheduler congestion — nudge to `12 18 * * *` once DST ends in November to hold 1:12pm ET) and calls `POST /api/approval-digest` directly (with the `X-Cron-Secret` header, from a `DIGEST_CRON_SECRET` secret set on this Worker). Also has a `fetch` handler running the same logic, so visiting the Worker's own URL triggers (and shows the result of) an on-demand run without waiting for the cron.
