...
As of 2026-09, this exists for **Solo Ringer** (`calcRinger`) and **Escalator of Doom** (`calcEscalatorDoom`) only. Every other format function still just does `if (!isCardComplete(card)) continue;` and silently omits anyone mid-round — safe (no crash, no bad numbers, thanks to the `isCardComplete` fix above) but invisible until they finish. Extending this to another format is real work, not a flag flip — each format's scoring mechanic interacts with "partial data" differently. Read both existing implementations before starting; which one to follow depends on `format.teamSize`:
...
- A partial team can still get a meaningful **provisional to-par**, computed from `completePlayers` only (not live/in-progress individual rounds — those can still get worse before that player finishes). For each hole, `countN = min(segmentCount, completePlayers.length)` — the segment's normal best-of-N count capped to how many are actually done — and scale that hole's par by the *same* `countN` so the comparison stays fair at every stage. This is exactly `calcEscalatorDoom`'s real scoring formula with `completePlayers.length` standing in for `teamSize` until the roster's actually full, so it converges to the real number once everyone's in. Label it "Provisional" wherever it's shown (see the `team.inProgress` row in `renderTeamLeaderboard`) — it must never look like a ranked score.
- `applyPayouts()` iterates every result in `results`, including in-progress ones — an in-progress team result has no `players` array (it has `members` instead), so a payouts list keyed by player name (`{ place, player, amount }`, as opposed to one keyed by position) will throw on `r.players.filter(...)`. `applyPayouts` now skips any `r.inProgress` entry up front (prize stays `null`, which is already the constructed default) — keep that guard if this function is touched again.
