...
- **Protected writes — `functions/admin/api/*`** (route `/admin/api/*`, behind Cloudflare Access):
  ...
  - `/admin/api/players` — onboard/add/remove player, refresh handicaps, `set-discord` (map a player to their numeric Discord user ID in `players:discord`, used for `<@id>` mentions in results posts, League Alert, and the approval digest)
