...
### 9c. Week-over-Week Handicap Changes (admin, added 2026-09)
- **The data already existed, it just wasn't being read.** `events.js`'s `activate-event` action has snapshotted the entire `players:handicaps` blob verbatim under `${eventId}:handicaps` (event's KV `id`, not `tournamentId`) since before this feature — a locked-in record of every player's MashCAP at the moment that event went live. It was purely write-only until now, kept only as a backup artifact (see **Backup & Disaster Recovery** below). No snapshot exists for an event that's never been Activated.
- **`GET /admin/api/handicap-changes?eventId=<id>`** — Access-protected. Resolves the event's `season`/`week`, finds that season's **previous week** and **week 1** siblings (by `week` number, not chronological order), loads all three snapshots, and diffs each player's `mashCap` (`current - prevWeek`, `current - week1`). A player missing from a comparison snapshot (new to the roster, or that week's event was never activated) gets `null` deltas rather than a false zero. For Week 2 specifically, "previous week" and "week 1" are the same event, so both columns naturally show identical numbers — this isn't a bug, it's correct given only one prior week exists.
- **`admin/handicap-changes.html?eventId=<id>`** — table sorted by biggest absolute week-over-week move first (so notable jumps surface without scrolling), red-up/green-down since a rising MashCAP is a worse handicap. Linked from each active/completed event's row on `admin/events.html` as "📈 Handicaps".
- **Deliberately admin-only** — MashCAP movement is exactly the kind of number that invites arguments about sandbagging if shown publicly; kept behind Cloudflare Access alongside every other admin tool.
...
- **No player cap.** Season 10 launched open-enrollment by choice.
- **Closing registration (done 2026-09, once Season 10 started):** two independent halves, both needed — a client-only close is trivially bypassed by POSTing to the API directly. (1) `registration.html`'s `REGISTRATION_CLOSED` flag swaps the whole page from `#open-state` (the real form) to `#closed-state` (a short notice + a "notify me for next season" mini-form — name, email, optional note, no SGT lookup). (2) `functions/api/register.js`'s `CLOSED_SEASONS` Set actually rejects the POST server-side (403) — keep the two in sync by hand, there's no shared config between a static page and a Function. The **Register** nav link was also removed from every page carrying the duplicated nav — see **Nav rollout** below, which also corrects the stale file count this bullet used to have.
- **"Notify me" interest list:** `POST /api/interest` (mirrors `/api/register`'s shape — CORS, rate limit, one KV key per submission, optional Discord ping) stores `{name, email, note}` under `interest:<bucket>:<id>` (bucket defaults to `season-11`, validated against the same `season-\d+` pattern as registrations). **Not a real registration** — no review/approve workflow, no roster integration. Admin views/removes entries at `/admin/interest.html` via `GET`/`DELETE /admin/api/interest` (same `requireAccess` Cloudflare Access gate as every other admin endpoint).
...
- **Nav rollout:** the 8 pages carrying the duplicated hamburger nav (`index`, `about`, `requirements`, `rules`, `season`, `event`, `event-teams`, `recap` — see the hamburger-nav-duplication gotcha elsewhere in this doc) list **Season 10** above a divider, then **Season 9 (Archive)** below it — **Register for Season 10** used to sit between them but was removed from all 8 when registration closed (2026-09). Re-add it (or an equivalent Season 11 link) in all 8 when the next season opens — there's no shared nav partial, so this really is 8 separate edits every time.
...
- **Local dev:** `serve.mjs` is documented elsewhere as proxying `/api/*` to production, but as of 2026-09 that proxy 502s locally (`proxy failed: fetch failed`) — verify against the current file before relying on it; static `data/*.json` fetches (scorecards, events, seasons) work fine locally regardless, which is enough to test any page whose event already lives in the static files.

---

## Event Summary (`js/event-summary.js` engine)

A post-event stat digest — separate from the season-wide `js/stats.js` engine above, this one is scoped to a single event. Started 2026-09, v1 only; the user's own framing was "let's get started and then iterate," so treat the metric set as a first draft, not a fixed spec.

- **Page:** `event-summary.html?id=<tournamentId>` — linked from `event.html`'s header action row as "📊 Event Summary", shown only once `event.status === 'completed'` (and never for `format.type === 'invitational'`, which has no scorecards). Computed live on every page load from `fetchScorecards()` — same philosophy as `recap.html`/`season.html`: cheap enough to recompute, nothing to keep in sync, no stored/pre-baked summary. "Automatically create" (the original ask) is satisfied by it always being current and one click away once the event closes, not by a batch job that runs at closure time — there's no cron/workflow step here, just a page that's always accurate whenever visited.
- **Engine:** `js/event-summary.js` exports one function, `buildEventSummary(scorecards, format)` → `{ meta, eventStats: {gross, net}, playerStats: {gross, net, mostImproved}, teamStats }`. Pure, DOM-free, same convention as `scoring.js`/`stats.js`.
- **Classification convention deliberately reuses `holeScoreClass()`'s diff buckets** (eagle: diff≤-2, birdie: diff===-1 exact, par: 0, bogey: 1, double+: diff≥2) rather than inventing a new one — this keeps event-summary numbers consistent with `records.html`'s "Most Birdies" (which also uses exact birdies via `scanScorecardRounds`) and with how `event.html` colors scorecards. "Birdie-or-better" (used for streaks and the event-wide "most combined birdies" team stat) is computed separately as `diff <= -1`, not reusing the exact-birdie bucket.
- **Event-wide (Gross/Net, computed independently):** avg score, eagles, birdies, pars, bogeys, double-or-worse, hardest/easiest hole (by average to-par across the field).
- **Player spotlights (Gross/Net, computed independently):** best individual round, most birdies-or-better in a round, longest birdie-or-better streak (consecutive holes within one round — doesn't span across rounds), lowest front 9, lowest back 9. Plus **Most Improved** (round 2 net vs round 1 net) — gated on `numRounds >= 2`, so it only appears for multi-round formats like Solo Ringer, not single-round team formats.
- **Team stats (`format.teamSize > 1` only):** teams are re-resolved directly from each card's `TeamPlayer1..N` fields (grouped by sorted lowercase names) — this is a closed/completed event, so there's no need for the live-tracking roster machinery in `calcEscalatorDoom`/`buildTeamRoster`, just "did all `teamSize` members post a complete card." Metrics: best team gross/net aggregate (pure stroke sum, independent of that format's actual scoring rule — e.g. for Escalator of Doom this is a different number than the format's own best-ball total), most combined birdies-or-better (gross/net), most balanced team (smallest gap between best/worst individual net score), most carried team (largest such gap, naming the carrier and the weakest link).
- **Verified against real production data**: S9W1 (`event-50774`, Solo Ringer, 2-round) and S9W2 (`event-51554`, Escalator of Doom, 1-round team) — both already in the static `data/events.json`/`data/scorecards/`, so no mock harness was needed, just point the page at them directly.
- **Not yet built:** more player/team stat ideas (bounce-back rate, consistency/variance, biggest single-hole blowup, CTP-hit tie-ins).
- **Pace of Play (Total Time Played / Fastest / Slowest Round), added 2026-09** — reads every stream URL submitted for the event via the existing `/api/get-streams?eventId=X`, then resolves each *unique* video's length via `/api/youtube-duration?videoId=X`. **Season 10 only** — S9 and earlier never captured YouTube links at all, so `event-summary.html` checks `event.season` and skips this section entirely below season 10 rather than showing an empty/broken block.
  - **Attribution rule**: a video's duration is divided by however many `(player, round)` slots reference it. One rule covers two real cases — a Solo Ringer player who recorded both rounds in one continuous video (2 slots, same player, different rounds → each round gets half), and a team's shared-camera stream for their one round (N slots, same round, different players → each player's estimated time is 1/N). Shares of one video always sum back to exactly that video's own length, so "Total Time Played by All Players" is simply the sum of each *unique* video's duration — a shared stream is never double-counted.
  - **The fan-out happens in the browser, not the server** — this is the load-bearing design decision here, learned the hard way. A first version tried to do everything (list streams, then fetch every video's duration) inside one server-side Cloudflare Function call, and it hit **"Too many subrequests by single Worker invocation"** — S10W1 alone has 41 unique stream videos across 43 submitted URLs, and just resolving those 43 KV keys already uses most of the per-invocation subrequest budget (same class of limit `admin/backup.html`'s own comments already document hitting before). `/api/youtube-duration` therefore only ever handles **one video per call** (cache check → optional YouTube fetch → optional cache write, ≤3 subrequests) and `event-summary.html`'s `loadPaceOfPlay()` calls it once per unique video with `Promise.all` — each call is an independent Worker invocation with its own fresh subrequest budget, so there's no ceiling on how many videos one event can have. **If this pattern is reused for anything else that fans out over KV data, size the per-item work to fit in a handful of subrequests and let the client (or `Promise.all`) drive the fan-out — never loop a per-item external-API-plus-cache round trip inside one server invocation.**
  - Video durations are cached forever in KV (`yt-duration:<videoId>`) since a published video's length never changes — after the first view of an event, its videos' durations cost zero further YouTube API quota even across repeat page loads or other events reusing the same stream.

### Results Poster (admin, added 2026-09)
A PNG "poster" version of an event's final results, generated in the same **Post Results to Discord** flow that already existed on `admin/events.html` (the "📣 Results" button on a completed event's row) — this didn't need a new page, just extending one that was already computing the right data.
- **Reuses two things that already existed rather than inventing new infrastructure:** `html2canvas` (already loaded for `admin/poster-preview.html`'s pre-event announcement poster — same library, same `#poster`-div-to-canvas approach) and `functions/admin/api/announce.js` (already accepts `{ message, imageBase64 }` together and posts a multipart file+text message to Discord — its own code comment had literally anticipated "e.g. event results" as a future use case before this existed). **No backend changes were needed at all** — everything is client-side poster HTML + the existing endpoint.
- **PDF was considered and rejected**: Discord doesn't preview PDFs inline (they just show as a file link, killing the "poster" effect they're meant for), and there's no build pipeline in this repo for a PDF library like jsPDF anyway. PNG-only, matching the existing announcement-poster precedent exactly.
- **`generateResultsMessage()` was refactored**, not duplicated: it used to fetch scorecards and run `applyFormat`/`applyPayouts` internally and just return text. That's now split out into `computeEventResults(event, format)` (shared, returns `{ ev, results, scorecards }`) so the text message and the poster image are built from **the exact same computed results** in one fetch/score pass — they can never disagree with each other.
- **One highlight stat** pulled from `buildEventSummary()` (the Event Summary engine above) via `buildHighlight()`: prefers the longest net birdie-or-better streak, falls back to gross streak, falls back to best net round if nobody strung together a real streak. This is a deliberately different data source than the payouts/CTP data (which comes straight from `event.payouts`/`event.ctp`, already player-resolved by the time an event is completed) — the poster is the first place these two engines' outputs appear together on one artifact.
- **Correctly handles a real historical edge case**: a player's poster *position* (`r.position`, from `applyFormat`'s actual scoring) can differ from their payout's list-index `place` when an admin has manually adjusted payouts (e.g. S9W1's "tjak excluded from payout, remaining places shifted up" — GIL3S's true finish stayed 3rd even though they now receive the money that would've gone to 2nd). The poster shows the real position, matching the pre-existing text-message behavior exactly — not a bug, verified against that real event.
- **Verified end-to-end** via a mock harness (mocked `/api/events-admin` to mark S9W1 as `completed`, let real static scorecards/formats/overrides load normally) simulating a click through the full flow: modal open → poster renders with real payouts/CTP/highlight → Download PNG completes with no JS errors.
...
| **YouTube Data API Key** | Cloudflare Pages → Settings → Environment Variables → `YOUTUBE_API_KEY` (set as **Secret**, not Text) | Unknown — Google API keys don't auto-expire, but rotate if leaked | Added 2026-09 for `/api/youtube-duration` (video length lookups feeding the Event Summary's Pace of Play stats). Created under the existing `mashup-admin` Google Cloud project — no need for a separate project per API. Restricted to YouTube Data API v3 only. Free quota is 10,000 units/day; a duration lookup costs 1 unit, and results are cached forever in KV, so normal usage never approaches the limit. |
...
`isCardComplete(card)` in `js/scoring.js` handles this — in order:
1. `status === 'Completed'` → done.
2. `status !== 'Pending'` → not done.
3. `activeHole != null && activeHole <= 18` → **not done**, still genuinely in progress. This check has to come before the "are all 18 holes present" check below, and it's the important one — see the gotcha next.
4. Otherwise, fall back to "every `hole{1-18}_net` is present" → done. This is the original SGT-leaves-it-on-Pending fix from above.

**The 0-vs-null gotcha (confirmed 2026-09, cost a live leaderboard showing -61):** while a round is genuinely in progress, SGT fills holes not yet played with a **literal `0`**, not `null`/missing. So "every hole has *a* value" was true the moment a player teed off at all, long before they finished — the old (step-3-less) version of this function treated a 3-holes-in round as complete, and the format functions scored the 0-placeholders as real (wildly-under-par) strokes. `activeHole` (1-18 while still playing that round, **19** once truly done — that's SGT's own "past hole 18" marker, not something this codebase invented) is the reliable "how far have they actually gotten" signal, which is why it's checked first. If `activeHole` is ever missing on a card, this falls back to the older heuristic — less precise, but better than crashing.

`cardHasStarted(card)` (right below `isCardComplete`) answers a different question — "has this player recorded *any* stroke yet" (any `hole{1-18}_net > 0`) — used to decide whether an incomplete round is worth surfacing as "in progress" in the UI at all, versus a card that's still all placeholder zeros because the player hasn't teed off.

All format-function completion checks go through `isCardComplete()` — if a similar gap ever shows up for some other status string, fix it here once, not per-format.

### Live / In-Progress Round Tracking — the recipe for a new format

As of 2026-09, this exists for **Solo Ringer** (`calcRinger`), **Escalator of Doom** (`calcEscalatorDoom`), **Best Ball 3-Man** (`calcBestBall3Man`), **Modified BB 3-Man** (`calcModifiedBB3Man`), **Best 2/Worst 2/All 3** (`calcBest2Worst2All3`), and **3-Man Modified Stableford** (`calcStableford3Man`). Every other format function still just does `if (!isCardComplete(card)) continue;` and silently omits anyone mid-round — safe (no crash, no bad numbers, thanks to the `isCardComplete` fix above) but invisible until they finish. Extending this to another format is real work, not a flag flip — each format's scoring mechanic interacts with "partial data" differently. Read at least one existing implementation before starting; which one to follow depends on `format.teamSize`:

**Not yet covered, and why:** Nassau and Shamble (teamSize 2) are straightforward extensions of the same pattern, just not done yet. Devil's Draw (3-man and 4-man) can get the roster-visibility half (show the team, all-grey, pre-play) but not a provisional score — the hole-to-category draw happens live *after* everyone finishes, so there's no fixed segment rule to compute a partial total from mid-round. Lone Ranger is genuinely harder: it's not "best N of M," it's a specific declared player mandatory on specific holes by a pre-set slot rotation, so a provisional number would need to know whose turn it is, not just how many are done.

**Individual formats (teamSize 1) — follow `calcRinger`'s pattern:**
- After building the normal ranked `results` from complete cards only, do a second pass over *all* scorecards looking for `status === 'Pending' && !isCardComplete(card) && cardHasStarted(card)`.
- If that player has **zero** complete rounds yet, push an unranked placeholder result (`inProgress: true, position: null, total: null`) carrying a synthetic partial round: `net`/`gross` arrays with `null` for every hole at/after `activeHole` (never the SGT `0`), `total: null`. `scoreCell()` in `event.html` already renders `null` as a blank dash instead of running it through `holeScoreClass` (which would otherwise paint a 0 as an eagle) — reuse that, don't reinvent it.
- If that player **already has** a complete-round result, don't skip them — append the live round onto their existing `.rounds` array instead (flag `hasLiveRound: true`, `liveHolesPlayed: N`) so the scorecard shows both. Whether the live round should also feed back into that player's *ranking* is genuinely format-specific: for Ringer, a played hole is settled data (won't get worse later in the same round), so it's fair to fold it into the ranking immediately — see `rankAndPosition()` below. A format without a "best-of-N-rounds" mechanic likely shouldn't let a live round touch its total/position at all; think through what "provisional" means for that format's actual scoring rule before copying this part.
- If ranking can change from live data, don't hand-roll the sort/tiebreak logic twice — extract it into a `rankAndPosition(list)` helper (mutates `.position`/`.tied` in place) so it can run once initially and again after any live improvement, same as `calcRinger` does.
- **Showing the whole field, not just whoever's posted a card (added 2026-09):** individual formats have no `event.teams`-style roster of their own, so `calcRinger` takes a 3rd `event` param and reads `event.players` — the season roster, attached by `event.html` (`{ ...event, players: season?.players }`, resolved from `loadSeasons()` before `applyFormat` runs, since it wasn't needed before this). Anyone in that list not already covered by a ranked or in-progress result gets pushed as `{ player_name, notStarted: true, position: null, total: null, toPar: null }`, appended *after* the in-progress entries so the final order is ranked → in-progress → not-started. `renderSoloLeaderboard`'s `player.notStarted` branch renders a dim, unranked, non-clickable row (no scorecard to expand into). The `event?.players?.length` guard means this only activates where `event.html` explicitly attaches it — `recap.html`/`season.html`'s own `applyFormat` calls never set `.players` on their event objects, so a completed archive event never grows padding rows for a season roster that doesn't even apply to it.

**Team formats (teamSize > 1) — follow `calcEscalatorDoom`'s pattern:**
- Teammates can each finish hours apart, so "who's on this team" can't be inferred only from whoever already has a card. Seed the **full roster** from `event.teams` (the admin draw) first — every named member starts as `{ name, status: 'not-started' }` — *then* layer in real status from scorecards (`complete` / `live` with `holesPlayed` / stays `not-started`). This is the only way a member with zero cards at all still shows up instead of the team just looking short-handed.
- `event.teams` doesn't exist for every event (older ones, or before the admin's entered this week's draw), so don't rely on it alone. SGT's own `TeamPlayer1/2/3` fields already name the *whole* team on every single card — including teammates who haven't submitted their own round yet — so also seed `not-started` placeholders from `resolveTeamKey`'s `displayMembers` for every card processed (guarded by `!team.memberStatus.has(...)` so it never clobbers a real status already recorded). Missing this step was a real bug: `team.displayMembers` (used for the header/name list) picked up all 3 names from one card, but `team.memberStatus` (what the leaderboard row actually renders) only ever got an entry for that card's own owner — so a not-yet-started teammate was invisible even though their name was sitting right there in the data.
- All of the above (roster-seeding, per-member status, substitutions) is shared across every team format via **`buildTeamRoster(scorecards, event, teamSize)`** in `js/scoring.js` — don't re-implement it per format. It returns `{ key -> { displayMembers, memberStatus: Map, pars, indices } }`; each `calc*` function then does its own scoring from `[...team.memberStatus.values()].filter(m => m.status === 'complete')`.
- **Don't score a team until every roster spot is `complete`.** This was a real latent bug found while building the Escalator of Doom version, not just a missing feature: the old code scored using `team.players.length` (however many members happened to be done), so a 2-of-3-complete team would silently produce a real-looking but *wrong* total for whichever segment ("all N score" holes) depends on the true team size. Gate the scoring block on `completePlayers.length === teamSize`; short of that, push an unranked `{ isTeam: true, inProgress: true, members: [...] }` entry instead.
- A team with **zero** activity (nobody on it has started) still gets that unranked entry — don't gate it behind "at least one member has started." Seeing the full roster before anyone tees off (who's on which team, even all-grey) was explicitly asked for once it became possible via `event.teams`; the entry is genuinely known at that point, so there's no reason to hide it. This only actually shows all teams pre-play when `event.teams` is populated — a team discovered purely from a scorecard's `TeamPlayer1-3` fields still can't appear before that scorecard exists.
- Render the in-progress team as one row listing every member's own status (a colored dot per member reads better than a single team-wide badge) plus a `${doneCount}/${teamSize} IN` summary chip — see `renderTeamLeaderboard`'s `team.inProgress` branch in `event.html` for the exact chip markup (green dot = complete, pulsing amber = live w/ "(thru N)", dim = not started).
- A partial team can still get a meaningful **provisional to-par**, computed from `completePlayers` only (not live/in-progress individual rounds — those can still get worse before that player finishes). For each hole, `countN = min(segmentCount, completePlayers.length)` — the segment's normal best-of-N count capped to how many are actually done, using whatever selection rule that format's real scoring uses (lowest-N for a best-ball segment, highest-N for a "worst 2" segment like Best2/Worst2/All3's par-4 holes, etc.) — and scale that hole's par by the *same* `countN` so the comparison stays fair at every stage. This is exactly the format's real scoring formula with `completePlayers.length` standing in for the segment's nominal count until the roster's actually full, so it converges to the real number once everyone's in. Label it "Provisional" wherever it's shown (see the `team.inProgress` row in `renderTeamLeaderboard`) — it must never look like a ranked score. **Stableford is the one exception**: its `toPar` field is repurposed to hold raw points (higher is better, no par offset), so the provisional there is a plain point total, not a to-par diff — flag the in-progress result with `isStableford: true` so `renderTeamLeaderboard` shows "N pts" instead of running it through `formatToPar()`.
- **Any function that iterates team results and touches `.players` must filter out `.inProgress` entries first** — an in-progress result has `members`, not `players` (the roster with per-member status vs. the final scored lineup), so `.players` is `undefined` on it. This exact crash has shown up three separate times as new formats/call sites were added: `applyPayouts()` (player-keyed payouts branch), `buildTeamSidePotHtml()`, and `buildStablefordSidePotHtml()` in `event.html` — all three now do `results.filter(r => !r.inProgress)` (or an equivalent per-row check) before touching `.players`. Any *new* function written against `results` that assumes every entry is a fully-scored team needs the same guard.

**Other things every version of this touches, regardless of format:**
- The leaderboard row needs its stream-icon cell wired up too (`getPlayerStreams`/`streamIconCell`) — it's easy to build the in-progress row without it and end up with a live player who's already submitted a stream link showing no icon at all (this happened once, see event.html's `renderSoloLeaderboard`).
- An already-ranked player/team with a later round or member still live gets a small `chip chip-live` + `pulse-dot` "LIVE" badge next to their name/team, distinct from the bigger unranked "IN PROGRESS" treatment for someone with nothing complete yet.
- `js/scoring.js` changed → bump the `?v=` query string on **every** page that imports it (`event.html`, `recap.html`, `season.html` as of 2026-09) — see **Frontend Conventions** below for why this matters and has bitten this project before.
- There's no real production data for most formats yet this season (only Solo Ringer and, soon, Escalator of Doom have actually run live) — verify a new format's live-tracking logic with hand-built synthetic scorecards (mirroring the shape of a real fetched card: `status`, `round`, `activeHole`, `hole{1-18}_net/gross`, `TeamPlayer1-3` or `event.teams`) rather than waiting for it to happen live.
...
- **Hamburger nav menu (`<div id="nav-menu">`) is copy-pasted, not shared — update all 8 copies together.** Only pages with the full hamburger button + dropdown carry it: `index.html` (the canonical copy — always update this one first), `about.html`, `event-teams.html`, `event.html`, `recap.html`, `requirements.html`, `rules.html`, `season.html`. (Simpler standalone pages — `records.html`, `player.html`, `power.html`, `handicaps.html`, `hole-in-one.html`, `registration.html`, etc. — use a plain "← Back to Site" header instead and don't have this menu at all; that's intentional, leave them alone.) There's no shared include, so adding/renaming/reordering a nav item on one copy silently does nothing on the other 7 — confirmed 2026-09, four items (Handicaps, Power Rankings, Records & Hall of Fame, Hole-in-One Pot) had only ever been added to `index.html`'s copy and were missing everywhere else for long enough that a user noticed. **When touching this menu: grab the current block from `index.html` and paste it verbatim into the other 7** (same flat directory, so the same relative `href`s work unchanged everywhere) rather than hand-editing each one.
...
| A hamburger-menu link is missing on some pages but not others | Nav menu is copy-pasted across 8 files, not shared (see Frontend Conventions) | Copy `index.html`'s current `<div id="nav-menu">` block into the other 7 verbatim |
