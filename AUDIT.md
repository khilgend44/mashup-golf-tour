...
### 🟡 P1-6b — Same race, same fix, in dues.js — ✅ FIXED 2026-09-04
Same shared-blob read-modify-write pattern as P1-6, found in a second file: `dues:<season>` was one JSON blob for every player's dues. Also confirmed happening in production — checking a player's "Paid" checkbox on `admin/player-data.html` also auto-fills Date Paid in the same click, firing two saves back-to-back with no atomicity, and one would silently clobber the other (dropping `paid`, which is also why the dues summary/count wouldn't update, and why the entered data was gone on refresh). Fixed the same way as P1-6: one KV key per player per season (`dues:<season>:<lowercaseUsername>`), with `admin/api/dues.js`'s GET auto-migrating any leftover data under the old blob key. Also added a client-side per-username write queue in `admin/player-data.html` (shared by both `setField`/`players:meta` and `setDuesField`/`dues`) so two saves for the same player can never run concurrently from one browser tab, regardless of what triggers them — closes this off generally, not just for the one paid+date case that surfaced it.
**Known follow-up, not yet fixed:** `players:meta` (`functions/admin/api/player-meta.js`) has the identical shared-blob pattern and hasn't been restructured — the client-side write queue above mitigates it for single-tab use, but two admins in different tabs (or the registration-approval flow in `registrations.js`, which does its own independent read-modify-write on `players:meta`) could still race. Left alone for now since it's more invasive to fix correctly (the bulk `import` action and the approve-flow's own write both need to move to the same per-player-key model to stay consistent), and no failure has actually been observed here yet.
